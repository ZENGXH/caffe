...
class CpmDataTransformer {
 public:
  explicit CpmDataTransformer(const HPTransformationParameter& param, Phase phase);
  ...
  void Transform_bottomup(const string& image_name, const string& root_folder, const vector<vector<float> >& data, Blob<Dtype>* transformed_data, Blob<Dtype>* transformed_label);


  void InitRand(int rng_seed) {
...
protected:
  ...
  void Transform_bottomup(const string& image_name, const string& root_folder, const vector<vector<float> >&, Dtype* transformed_data, Dtype* transformed_label);
  void ReadMetaData_bottomup(MetaData& meta, const vector<vector<float> >& data, size_t offset3, size_t offset1);
