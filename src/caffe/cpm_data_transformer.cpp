...
namespace caffe {
    ...
    template<typename Dtype>
        CpmDataTransformer<Dtype>::CpmDataTransformer(const HPTransformationParameter& param, Phase phase): param_(param), phase_(phase) {
        ...
        void CpmDataTransformer<Dtype>::swapLeftRight(Joints& j) {
            ...
            template<typename Dtype>
                void CpmDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta, Dtype* mask, Mat& teeth_mask) {
                    ...
                    LOG(FATAL) << "label cleaned -- meta type" << meta.type;
                    ...
                    if(meta.type == "cpm"){
                        ...
                        for (int i = 0; i < np; i++){
                            ...
                            if(meta.joint_self.isVisible[i] <= 1){
                                putGaussianMaps(transformed_label + i*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma()); //self
                                putGaussianMaps(transformed_label + (i+np+1+mask_channel)*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma()); //self
                            }
                            ...
                            for(int j = 0; j < meta.numOtherPeople; j++){ //for every other person
                                ...
                                if(meta.joint_others[j].isVisible[i] <= 1){
                                    putGaussianMaps(transformed_label + (i+np+1+mask_channel)*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma());
                                }
                            }
                        }
                        ...
                        for (int g_y = 0; g_y < grid_y; g_y++) {
                            for (int g_x = 0; g_x < grid_x; g_x++) {
                                float maximum = 0;
                                for (int i = 0; i < np; i++){
                                    maximum = (maximum > (float)transformed_label[i*channelOffset + g_y*grid_x + g_x]) ? maximum : (float)transformed_label[i*channelOffset + g_y*grid_x + g_x];
                                }
            ...
            template<typename Dtype>
                void CpmDataTransformer<Dtype>::Transform_CPM(const Datum& datum, Blob<Dtype>* transformed_data, Blob<Dtype>* transformed_label, Blob<Dtype>* mask, int cnt) {
                    ...
                    LOG(INFO) << "datum shape: " << datum_channels << " " << datum_height <<  " " << datum_width;
                    ...
                    LOG(INFO) << transformed_data->num() << " " << transformed_data->channels() << " "
                        << transformed_data->height() << " " << transformed_data->width();
                    LOG(INFO) << "label shape: " ;
                    LOG(INFO) << transformed_label->num() << " " << transformed_label->channels() << " "
                    ...
                    if(this->param_.has_masks()){
                        ...
                        for (int i = 0; i < img.rows; ++i) {
                            for (int j = 0; j < img.cols; ++j) {
                                ...
                                if (has_uint8)
                                    m = static_cast<uchar>(static_cast<uint8_t>(data[dindex]));
                                else
                ...
                for (int j = 0; j < img.cols; ++j) {
                    ...
                    for(int c = 0; c < 3; c++){
                        ...
                        if (has_uint8)
                            ...
                            d_element = datum.float_data(dindex);
                        rgb[c] = static_cast<float>(d_element);
                    }
                }
            }
            Mat teeth_mask, teeth_mask_temp1, teeth_mask_temp2, teeth_mask_temp3, teeth_mask_aug;
...
// subcall bottomup
template<typename Dtype>
void CpmDataTransformer<Dtype>::Transform_bottomup(const string& image_name, const string& root_folder, const vector<vector<float> >& human_pose_vec,  Blob<Dtype>* transformed_data, Blob<Dtype>* transformed_label) {
    Dtype* transformed_data_pointer = transformed_data->mutable_cpu_data();
    Dtype* transformed_label_pointer = transformed_label->mutable_cpu_data();

    Transform_bottomup(image_name, root_folder, human_pose_vec, transformed_data_pointer, transformed_label_pointer); //call function 1
}

template<typename Dtype>
void CpmDataTransformer<Dtype>::ReadMetaData_bottomup(MetaData& meta, const vector<vector<float> >& human_pose, size_t offset3, size_t offset1) { //very specific to genLMDB.py
    // ------------------- Dataset name ----------------------
    // ------------------- Image Dimension -------------------

    float height, width;
    meta.numOtherPeople = human_pose.size() - 1;
    // (int)data[offset3+2*offset1+1];
    float annolist_index;
    meta.annolist_index = 0; //(int)annolist_index;

    // count epochs according to counters
    static int cur_epoch = -1;
    meta.write_number = cur_epoch;
    VLOG(3) << "first perple " << human_pose[0].size();
    meta.bbox.left = human_pose[0][0] - 1;
    meta.bbox.top = human_pose[0][1] - 1;
    meta.bbox.width = human_pose[0][2];
    meta.bbox.height= human_pose[0][3];

    meta.scale_self = max(meta.bbox.width/this->param_.crop_size_x(), meta.bbox.height/this->param_.crop_size_y());
    meta.objpos = Point2f(meta.bbox.left + meta.bbox.width/2, meta.bbox.top + meta.bbox.height/2); 
    int offset = 4;
    int np_in_lmdb_real = this->param_.np_in_lmdb();

    meta.joint_self.joints.resize(np_in_lmdb_real);
    meta.joint_self.isVisible.resize(np_in_lmdb_real);
    for(int i=0; i<np_in_lmdb_real; i++){
        meta.joint_self.joints[i].x = human_pose[0][3*i+offset];
        meta.joint_self.joints[i].y = human_pose[0][3*i+1+offset];
        meta.joint_self.joints[i] -= Point2f(1, 1); 
        int isVisible = int(human_pose[0][3*i+2+offset]);
        if (isVisible == 3){
    ...
    for(int p=0; p<meta.numOtherPeople; p++){
        VLOG(3) << "people: " << p << " with size " << human_pose[p+1].size();
        meta.bboxes_other[p].left = human_pose[p+1][0] - 1;
        meta.bboxes_other[p].top = human_pose[p+1][1] - 1;
        meta.bboxes_other[p].width = human_pose[p+1][2];
        meta.bboxes_other[p].height= human_pose[p+1][3];
        meta.scale_other[p] = max(meta.bboxes_other[p].width/this->param_.crop_size_x(), meta.bboxes_other[p].height/this->param_.crop_size_y());
        meta.objpos_other[p] = Point2f(meta.bboxes_other[p].left + meta.bboxes_other[p].width/2, meta.bboxes_other[p].top + meta.bboxes_other[p].height/2); 
    ...
    //8 + numOtherPeople lines loaded
    for(int p=0; p<meta.numOtherPeople; p++){
        meta.joint_others[p].joints.resize(np_in_lmdb_real);
        meta.joint_others[p].isVisible.resize(np_in_lmdb_real);
        for(int i=0; i<np_in_lmdb_real; i++){
            meta.joint_others[p].joints[i].x = human_pose[p+1][3*i+offset];
            meta.joint_others[p].joints[i].y = human_pose[p+1][3*i+1+offset];
            meta.joint_others[p].joints[i] -= Point2f(1,1);
            int isVisible = int(human_pose[p+1][3*i+2+offset]);
    ...
    if(this->param_.debug()){
        ofstream myfile;
        myfile.open("COCO_train_list_2.txt", ios::out | ios::app);

        char filename[100];
        sprintf(filename, "%s_%d.jpg", meta.file_name.c_str(), meta.numOtherPeople+1);
        myfile << filename << " ";
...
template<typename Dtype>
void CpmDataTransformer<Dtype>::Transform_bottomup(const string& image_name, const string& root_folder, const vector<vector< float > >& human_pose_vec , Dtype* transformed_data, Dtype* transformed_label) {
    //TODO: some parameter should be set in prototxt
    int clahe_tileSize = this->param_.clahe_tile_size();
    int clahe_clipLimit = this->param_.clahe_clip_limit();
    //float targetDist = 41.0/35.0;
    ...
    //before any transformation, get the image from datum
    VLOG(3) << "loading" << image_name;
    cv::Mat img = ReadImageToCVMat(root_folder + image_name);
    VLOG(4) << "    done loading" << image_name;
    const int datum_height = img.rows;// datum.height();
    const int datum_width = img.cols; // datum.width();
    CHECK(img.rows > 0 && img.cols > 0) << " get empty image " << image_name;
    cv::Mat mask;

    if(param_.need_binary_mask()){
        bool is_color = false;
        mask = ReadImageToCVMat(root_folder + "/../train2014mask/mask_"+ image_name, is_color);
        CHECK(mask.rows == img.rows && img.cols == mask.cols) << " get empty image " << image_name;
    }else{
        mask = Mat::zeros(img.rows, img.cols, CV_8UC1);
    }
    VLOG(3) << "load mask done";
    Mat mask_all, mask_miss;
    //if(mode >= 5){
    mask_miss = mask.clone(); // Mat::ones(datum_height, datum_width, CV_8UC1);
    //}
    //if(mode == 6){
    mask_all = mask.clone(); // Mat::zeros(datum_height, datum_width, CV_8UC1);
    //}
    ...
    int offset1 = datum_width;
    int stride = this->param_.stride();
    VLOG(3) << "ReadMetaData_bottomup .." << human_pose_vec.size();
    MetaData meta;
    meta.img_size.width = img.cols;
    meta.img_size.height = img.rows;
    meta.file_name = image_name;
    CHECK(human_pose_vec.size() > 0) << "get size " << human_pose_vec.size() << image_name;
    ReadMetaData_bottomup(meta, human_pose_vec, offset3, offset1);
    VLOG(3) << "ReadMetaData_bottomup done";
    if(false){
        char name[100];
    ...
    //copy transformed img (img_aug) into transformed_data, do the mean-subtraction here
    offset = img_aug.rows * img_aug.cols;
    CHECK(img_aug.rows > 0 && img_aug.cols > 0) << " get empty aug image " << image_name;
    CHECK(mask_miss_aug.rows > 0 && mask_miss_aug.cols > 0) << " get empty aug image " << image_name;
    int rezX = img_aug.cols;
    int rezY = img_aug.rows;
    int grid_x = rezX / stride;
    int grid_y = rezY / stride;
    ...
    if(!this->param_.analysis()){
        ...
        for (int g_y = 0; g_y < grid_y; g_y++){
            ...
            VLOG(3) << "copy label done; image transformation done!";
    ...
    template<typename Dtype>
        float CpmDataTransformer<Dtype>::augmentation_scale(Mat& img_src, Mat& img_temp,
            ...
            if(!this->param_.use_segmentation_scale()){
                ...
                scale_abs = this->param_.target_dist() / meta.scale_self;
                if(meta.scale_self == 0){
                    VLOG(3) << "scale_abs == 0 -> 1"; 
                    scale_abs = 1;
                }
            } else {
                scale_abs = this->param_.target_dist() / sqrt(meta.segmentation_area); //bad.
                if(meta.segmentation_area == 0){
                    VLOG(3) << "scale_abs == 0 -> 1"; 
                    scale_abs = 1;
                }
            }
            ...
            VLOG(3) << "scale " << scale << " scale_abs * scale_multiplier " << scale_abs << " " << scale_multiplier ;
            VLOG(3) << "use_segmentation_scale? " << this->param_.use_segmentation_scale() << " scale_self " <<  meta.scale_self << "(h, w) " << meta.bbox.width << " " << meta.bbox.height;
            if(!this->param_.analysis()){
                resize(img_src, img_temp, Size(), scale, scale, INTER_CUBIC);
        ...
        void CpmDataTransformer<Dtype>::putVecMaps(Dtype* entryX, Dtype* entryY, Mat& count, Point2f centerA, Point2f centerB, int stride, int grid_x, int grid_y, float sigma, int thre, float peak_ratio){
            ...
            template<typename Dtype>
                void CpmDataTransformer<Dtype>::visualize(Mat& img, MetaData meta, AugmentSelection as, Mat& coco_mask) {
                    ...
                    rectangle(img_vis, Point2f(meta.bbox.left, meta.bbox.top),
                            Point2f(meta.bbox.left, meta.bbox.top) + Point2f(meta.bbox.width, meta.bbox.height),
            ...
            template<typename Dtype>
                void CpmDataTransformer<Dtype>::visualizeLabelMap(const Dtype* transformed_label, Mat& img_aug){ 
                    int rezX = img_aug.cols;
                    int rezY = img_aug.rows;
                    int stride = this->param_.stride();
                    int grid_x = rezX / stride;
                    ...
                    char imagename [100];
                    static int count = 0;
                    string file_name = "mask";
                    // plot joint mask
                    for(int i = njoint; i < njoint + (np + num_bg_map); i++){      
                        Mat label_map;
                        label_map = Mat::zeros(grid_y, grid_x, CV_8UC1);
                        //int MPI_index = MPI_to_ours[i];
                        //Point2f center = meta.joint_self.joints[MPI_index];
                        for (int g_y = 0; g_y < grid_y; g_y++){
                            ...
                            for (int g_x = 0; g_x < grid_x; g_x++){
                                label_map.at<uchar>(g_y,g_x) = (int)(transformed_label[i*channelOffset + g_y*grid_x + g_x]*255);
                                // printf("%f ", transformed_label[i*channelOffset + g_y*grid_x + g_x]*255);
                            }
                        }
                        ...
                        sprintf(imagename, "%s_c%d_joint_part_%d.jpg", file_name.c_str(), count, i);
                        count ++;
                    ...
                    for(int i = 0; i < njoint; i++){      
                        ...
                        // circle(label_map, center, 3, CV_RGB(255,0,255), -1);
                        // sprintf(imagename, "augment_%s_human_%d_%d_label_part_%02d.jpg", meta_vec[0].file_name.c_str(), int(meta.objpos.x), int(meta.objpos.y), i);
                        sprintf(imagename, "%s_c%d_limb_paf_%d.jpg", file_name.c_str(), count, i);
                        count ++;
                        VLOG(3) << "<save> LIMB vis - filename is " << imagename;
                        //        resize(paf_map, paf_map, Size(46, 46), 0, 0, INTER_LINEAR);
                        imwrite(imagename, paf_map);
                    }

                    file_name = "heatmap";
                    for(int i = njoint+57; i < njoint+57 + (np + num_bg_map); i++){      
                        Mat label_map;
                        label_map = Mat::zeros(grid_y, grid_x, CV_8UC1);
                        //int MPI_index = MPI_to_ours[i];
                        //Point2f center = meta.joint_self.joints[MPI_index];
                        ...
                        //circle(label_map, center, 3, CV_RGB(255,0,255), -1);
                        //sprintf(imagename, "augment_%s_human_%d_%d_label_part_%02d.jpg", meta_vec[0].file_name.c_str(), int(meta.objpos.x), int(meta.objpos.y), i);
                        sprintf(imagename, "%s_c%d_joint_part_%d.jpg", file_name.c_str(),count,  i); count ++;
                        //        resize(label_map, label_map, Size(46, 46), 0, 0, INTER_LINEAR);
                        imwrite(imagename, label_map);
                    } 

                    // draw limbs_x
                    for(int i = 0+57; i < njoint+57; i = i+2){      
                        Mat paf_map;
                        paf_map = Mat::zeros(grid_y, grid_x, CV_8UC1);
                        //int MPI_index = MPI_to_ours[i];
                        //Point2f center = meta.joint_self.joints[MPI_index];
                        ...
                        // circle(label_map, center, 3, CV_RGB(255,0,255), -1);
                        // sprintf(imagename, "augment_%s_human_%d_%d_label_part_%02d.jpg", meta_vec[0].file_name.c_str(), int(meta.objpos.x), int(meta.objpos.y), i);
                        sprintf(imagename, "%s_c%d_limb_paf_%d.jpg",  file_name.c_str(), count, i); count ++;
                        VLOG(3) << "<save> LIMB vis - filename is " << imagename;
                        //        resize(paf_map, paf_map, Size(46, 46), 0, 0, INTER_LINEAR);
                        imwrite(imagename, paf_map);
                    }
                ...
                void CpmDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta) {
                    ...
                    if (np == 37){
                        ...
                        for(int i = 0; i < 19; i++){
                            ...
                            if(jo.isVisible[mid_1[i]-1]<=1 && jo.isVisible[mid_2[i]-1]<=1){
                                //putVecPeaks
                                putVecMaps(transformed_label + (np+1+2*i)*channelOffset, transformed_label + (np+1+(2*i+1))*channelOffset, count, jo.joints[mid_1[i]-1], jo.joints[mid_2[i]-1], this->param_.stride(), grid_x, grid_y, this->param_.sigma(), thre, peak_ratio); //self
                            }
                        ...
                        for (int i = 0; i < 18; i++){
                            ...
                            if(meta.joint_self.isVisible[i] <= 1) {
                                putGaussianMaps(transformed_label + (np+1+38+i)*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma(), peak_ratio); //self
                            }
                            ...
                            for(int j = 0; j < meta.numOtherPeople; j++){ //for every other person
                                ...
                                Point2f center = meta.joint_others[j].joints[i];
                                if(meta.joint_others[j].isVisible[i] <= 1) {
                                    putGaussianMaps(transformed_label + (np+1+38+i)*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma(), peak_ratio);
                                }
                            }
                        }
                        ...
                        if(!this->param_.per_part_scale()){ // per_part_scale == false
                            ...
                            for(int i = 0; i < 18; i++){
                                ...
                                if(meta.joint_self.isVisible[i] <= 1) {
                                    putScaleMaps(transformed_label + (np+1+38+18)*channelOffset, center, this->param_.stride(), grid_x, grid_y, this->param_.sigma(), scale_normalize); //self
                                }
